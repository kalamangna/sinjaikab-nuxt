...
### Fixed
- Eliminasi pesan error `[Sync Sitemap Data] Network error (will retain existing snapshot): socket hang up` saat Vercel build dengan menambahkan deteksi variabel lingkungan `VERCEL` pada [`scripts/sync-sitemap-data.mjs`](./scripts/sync-sitemap-data.mjs): script kini otomatis di-skip saat berjalan di Vercel (runtime live fetch menangani pembaruan data), dan hanya dieksekusi dari lingkungan lokal.
- Perbaikan *Cumulative Layout Shift* (CLS) dari `0.021` menuju `0` pada halaman detail dokumen ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): (1) menghapus skeleton *Deskripsi Dokumen* dari loading state yang menyebabkan layout shift saat card tersebut tidak muncul pada data real (karena kondisional `v-if="dokumen.deskripsi"`), dan (2) mengganti URL background texture eksternal `transparenttextures.com` dengan SVG data URL inline agar tidak ada external resource yang dapat memicu layout shift.
- Perbaikan respons HTTP 404 yang benar saat dokumen PPID tidak ditemukan ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): menambahkan `watchEffect` dengan `createError({ statusCode: 404, fatal: true })` agar Google tidak mengindeks halaman kosong dengan status `200 OK`, sehingga halaman error Nuxt ([`error.vue`](./error.vue)) dirender dengan HTTP status code yang tepat.
- Peningkatan halaman error ([`error.vue`](./error.vue)): tampilan kini kontekstual berdasarkan kode error (404 vs lainnya) dan asal rute — jika dari halaman detail PPID, menampilkan tombol *"Kembali ke Daftar Dokumen"* beserta pesan yang relevan; jika dari rute lain, menampilkan tombol *"Kembali ke Beranda"* saja.
...
### Changed
- Optimasi dan standardisasi menyeluruh Meta Title & Meta Description SEO ([`app.vue`](./app.vue), [`pages/informasi/index.vue`](./pages/informasi/index.vue), dan [`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)):
  - **Beranda / Global ([`app.vue`](./app.vue))**: Memperbarui `pageTitle` default menjadi `"Pemerintah Kabupaten Sinjai - Bumi Panrita Kitta #samasamaki"` (58 karakter, pas batas SERP Google < 60 karakter) dan `pageDescription` menjadi 157 karakter yang selaras dengan narasi section Hero (`"Website resmi Pemerintah Kabupaten Sinjai #samasamaki. Pusat informasi program daerah, keterbukaan dokumen publik, dan layanan masyarakat Bumi Panrita Kitta."`), serta penyempurnaan `titleTemplate` agar cerdas menangani judul beranda dan sub-halaman tanpa duplikasi kata.
  - **Daftar Informasi Publik ([`pages/informasi/index.vue`](./pages/informasi/index.vue))**: Judul dan deskripsi kini dinamis merespons kata kunci pencarian aktif (`search`), jenis dokumen, serta kategori, dengan deskripsi default kaya kata kunci PPID & transparansi daerah (~153 karakter), dan sinkronisasi reaktif data terstruktur JSON-LD `CollectionPage`.
  - **Detail Dokumen ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue))**: Penambahan sanitasi `.trim()` pada judul, pemotongan cerdas deskripsi dokumen (*word-boundary truncation*) berelipsis sebelum batas 160 karakter untuk mencegah pemotongan kata di tengah huruf, fallback deskripsi informatif jika ringkasan dokumen kosong, serta pelengkapan tag `twitter:title` dan `twitter:description`.
...
- Penyederhanaan tampilan pratinjau dokumen ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): menghapus paragraf deskripsi dan tombol tautan berlebih pada placeholder berkas Google Drive serta memperpendek label tombol aksi menjadi *"Lihat Pratinjau"* untuk menciptakan tampilan visual yang bersih dan minimalis.
- Peningkatan kepatuhan kontras warna WCAG AA ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): menyesuaikan warna status sifat akses dokumen dari `text-emerald-600` menjadi `text-emerald-700` (rasio kontras $> 5.2:1$) dan subteks skeleton ke `text-slate-500` guna mengeliminasi seluruh temuan isu kontras Lighthouse.
- Optimasi pratinjau dokumen on-demand ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): menerapkan pemuatan bersyarat interaktif (*on-demand preview button*) untuk berkas iframe Google Drive guna mengeliminasi 14 third-party cookies dan pesan pelanggaran CSP framing (`frame-ancestors`) di browser console, sehingga meningkatkan skor audit Lighthouse Best Practices menuju 95-100.
- Optimasi performa webfont dan payload CSS ([`assets/css/fontawesome.css`](./assets/css/fontawesome.css) dan [`nuxt.config.ts`](./nuxt.config.ts)): menerapkan deklarasi `@font-face { font-display: swap; }` untuk mengeliminasi pemblokiran render 490ms pada audit Lighthouse, serta modularisasi pustaka FontAwesome (hanya mengimpor `fontawesome.min.css` dan `solid.min.css`) yang memangkas ukuran bundel CSS sebesar ~24 kB (~10 kB gzip).
- Peningkatan kontras warna aksesibilitas WCAG AA ([`pages/informasi/index.vue`](./pages/informasi/index.vue) dan [`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)): meningkatkan elemen berbobot kontras rendah (`text-slate-400` ke `text-slate-500` / rasio $> 4.5:1$) pada nomor urut tabel dokumen, fallback nama organisasi, tanggal kartu mobile, serta label metadata dokumen untuk meloloskan skor audit aksesibilitas (Lighthouse/Axe).
...
- Penambahan konfigurasi router resmi [`app/router.options.ts`](./app/router.options.ts) untuk memastikan navigasi antar halaman selalu otomatis *scroll to top* (`top: 0, left: 0`), serta penyelarasan tombol kembali pada [`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue) dan hook `onMounted` pada [`pages/informasi/index.vue`](./pages/informasi/index.vue) agar layar selalu kembali ke posisi paling atas saat berpindah dari halaman detail ke daftar dokumen.
...
### Changed
- Memperbaiki judul halaman (*document title*) pada rute `/admin/a11y-stats` menjadi `Statistik Aksesibilitas - Pemerintah Kabupaten Sinjai` serta menyematkan direktif meta `robots: noindex, nofollow` guna mencegah pengindeksan mesin pencari pada halaman internal.
