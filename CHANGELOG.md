...
### Changed
- Penyederhanaan Meta Title & Meta Description SEO ([`app.vue`](./app.vue), [`pages/informasi/index.vue`](./pages/informasi/index.vue), dan [`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue)):
  - **Beranda / Global ([`app.vue`](./app.vue))**: Mengubah `pageTitle` menjadi `"Pemerintah Kabupaten Sinjai"` (menghapus slogan dan tagar) dan `pageDescription` menjadi ringkas dan minimalis (`"Portal resmi Pemerintah Kabupaten Sinjai."`).
  - **Daftar Informasi Publik ([`pages/informasi/index.vue`](./pages/informasi/index.vue) & [`nuxt.config.ts`](./nuxt.config.ts))**: Menghapus aturan cache SWR statis pada rute `/informasi` agar parameter filter query string (seperti `?jenis_dokumen=IPKD` dan `?search=...`) dievaluasi dinamis via on-demand SSR; menyederhanakan format `pageTitle` dan `pageDescription` menjadi ringkas tanpa narasi panjang/berulang; serta menyelaraskan `og:url`, `og:title`, `twitter:title`, dan `twitter:description` agar tautan yang dibagikan ke WhatsApp dan media sosial menampilkan preview judul dan deskripsi yang sesuai dengan filter aktif.
  - **Detail Dokumen ([`pages/informasi/[slug].vue`](./pages/informasi/[slug].vue))**: Menghapus kalimat imbuhan dan frasa berulang pada deskripsi dokumen maupun fallback, menyajikan deskripsi ringkas dan to-the-point (`Dokumen <Judul> - Pemerintah Kabupaten Sinjai.`).
